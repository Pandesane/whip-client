...
export class ReferSubscriber extends EventEmitter {
  _id: any;
  _session: any;
  ...
  sendRefer(target: any, options: any = {}) {
    ...
    if (!extraHeaders.some((header: any) => header.toLowerCase().startsWith('referred-by:'))) {
  ...
  _requestSucceeded(response: any) {
  ...
  _requestFailed(response: any, cause: any) {
